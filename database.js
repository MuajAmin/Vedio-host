...
function getAllUserAvatars() {
    try {
        const rows = stmtUserAvatarsAll.all();
        const map = {};
...
function setUserAvatar(username, avatarFilename) {
    stmtUserAvatarUpsert.run(username, avatarFilename);
}
...
function isUserBlocked(username) {
    ...
    try {
        ...
        try {
            const row = stmtBlockedUserGet.get(username);
            return !!row;
        } catch {
            return false;
        }
...
function blockUser(username, reason = 'Blocked by admin') {
    if (!username) return;
    try {
        stmtBlockedUserUpsert.run(username, reason);
        if (blockedUsersCache) {
...
function getBlockedUsers() {
    try {
        return stmtBlockedUsersList.all();
    } catch {
        return [];
    }
}
...
function pruneExpiredSessions() {
    try {
        stmtSessionsPrune.run(Date.now());
    } catch (err) {
...
function getAllActiveSessions(currentSid = null) {
    try {
        const rows = stmtSessionsActiveAll.all(Date.now());
...
function destroyAllSessions(keepCurrentSid = null) {
    try {
        let result;
        if (keepCurrentSid) {
            result = stmtSessionsDestroyExceptCurrent.run(keepCurrentSid);
        } else {
...
function countUserSessions(username) {
    if (!username) return 0;
    try {
        const row = stmtUserSessionCount.get(username, Date.now());
...
const stmtUserAvatarsAll = db.prepare('SELECT username, avatar FROM user_profiles');
const stmtUserAvatarUpsert = db.prepare(`
...
const stmtBlockedUserGet = db.prepare('SELECT username FROM blocked_users WHERE username = ?');
const stmtBlockedUserUpsert = db.prepare(`
...
const stmtBlockedUsersList = db.prepare('SELECT username, reason, blocked_at FROM blocked_users');

const stmtSessionsPrune = db.prepare('DELETE FROM sessions WHERE expires_at <= ?');
const stmtSessionsActiveAll = db.prepare('SELECT sid, sess, expires_at FROM sessions WHERE expires_at > ? ORDER BY expires_at DESC');
...
const stmtSessionGet = db.prepare('SELECT sid, sess FROM sessions WHERE sid = ?');
const stmtSessionDelete = db.prepare('DELETE FROM sessions WHERE sid = ?');
const stmtSessionsDestroyExceptCurrent = db.prepare('DELETE FROM sessions WHERE sid != ?');
const stmtSessionsDestroyAll = db.prepare('DELETE FROM sessions');
const stmtUserSessionCount = db.prepare("SELECT COUNT(*) AS count FROM sessions WHERE json_extract(sess, '$.user') = ? AND expires_at > ?");

const stmtUserSettingsGet = db.prepare('SELECT ui_mode, theme FROM user_settings WHERE username = ?');
const stmtUserUiModeUpsert = db.prepare(`
...
`);
const stmtUserThemeUpsert = db.prepare(`
...
const stmtPushSubUpsert = db.prepare(`
...
function clearOldActivityLogs(username) {
    if (!username) return;
    try {
        stmtActivityLogsPruneUser.run(username, username);
    } catch (err) {
...
function setUserSetting(username, key, value) {
    ...
    try {
        ...
        if (key === 'ui_mode') {
            ...
            stmtUserUiModeUpsert.run(username, cleanVal, now);
            return true;
        }
        if (key === 'theme') {
            const cleanVal = String(value);
            stmtUserThemeUpsert.run(username, cleanVal, now);
            return true;
        }
        return false;
    } catch (err) {
...
function savePushSubscription(username, subscription, userAgent = null) {
    ...
    try {
        ...
        const nowIso = new Date().toISOString();
        stmtPushSubUpsert.run(username, subscription.endpoint, keys.p256dh || '', keys.auth || '', userAgent, nowIso, nowIso);
        return true;
    } catch (err) {
...
function pruneActivityLogs(maxKeep = 1500) {
    try {
        const result = stmtActivityLogsPruneAll.run(maxKeep);
        if (result.changes > 0) {
