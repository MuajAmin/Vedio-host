...
function getAllUserAvatars() {
    try {
        const rows = stmtUserAvatarGetAll.all();
        const map = {};
...
function setUserAvatar(username, avatarFilename) {
    stmtUserAvatarSet.run(username, avatarFilename);
}

function deleteUserAvatar(username) {
    stmtUserAvatarDelete.run(username);
}
...
function getBlockedUsersSet() {
    if (!blockedUsersCache) {
        const rows = stmtBlockedUsersGetAll.all();
...
function isUserBlocked(username) {
    ...
    try {
        ...
        try {
            const row = stmtBlockedUserCheck.get(username);
            return !!row;
        } catch {
            return false;
        }
...
function blockUser(username, reason = 'Blocked by admin') {
    if (!username) return;
    try {
        stmtBlockedUserSet.run(username, reason);
        if (blockedUsersCache) {
...
function unblockUser(username) {
    if (!username) return;
    try {
        stmtBlockedUserDelete.run(username);
        if (blockedUsersCache) {
...
function getBlockedUsers() {
    try {
        return stmtBlockedUsersGetList.all();
    } catch {
        return [];
    }
}
...
function pruneExpiredSessions() {
    try {
        stmtSessionsPruneExpired.run(Date.now());
    } catch (err) {
...
function getAllActiveSessions(currentSid = null) {
    try {
        const rows = stmtSessionsGetAllActive.all(Date.now());
...
function destroyUserSessions(username) {
    ...
    try {
        const result = stmtSessionsDestroyUser.run(username);
...
function destroyOtherUserSessions(username, currentSid) {
    ...
    try {
        let result;
        if (currentSid) {
            result = stmtSessionsDestroyOtherUser.run(username, currentSid);
        } else {
            result = stmtSessionsDestroyUser.run(username);
        }
        const count = result.changes || 0;
...
function destroySingleSession(sid) {
    if (!sid) return false;
    try {
        const row = stmtSessionGetBySid.get(sid);
        if (!row) return false;
        ...
        stmtSessionDestroySingle.run(sid);
...
function destroyAllSessions(keepCurrentSid = null) {
    try {
        let result;
        if (keepCurrentSid) {
            result = stmtSessionsDestroyAllExceptCurrent.run(keepCurrentSid);
        } else {
            result = stmtSessionsDestroyAll.run();
        }
...
function countUserSessions(username) {
    if (!username) return 0;
    try {
        const row = stmtSessionsCountUser.get(username, Date.now());
...
const stmtUserAvatarGetAll = db.prepare('SELECT username, avatar FROM user_profiles');
const stmtUserAvatarSet = db.prepare(`
...
`);
const stmtUserAvatarDelete = db.prepare('DELETE FROM user_profiles WHERE username = ?');

const stmtBlockedUsersGetAll = db.prepare('SELECT username FROM blocked_users');
const stmtBlockedUserCheck = db.prepare('SELECT username FROM blocked_users WHERE username = ?');
const stmtBlockedUserSet = db.prepare(`
...
`);
const stmtBlockedUserDelete = db.prepare('DELETE FROM blocked_users WHERE username = ?');
const stmtBlockedUsersGetList = db.prepare('SELECT username, reason, blocked_at FROM blocked_users');

const stmtSessionsPruneExpired = db.prepare('DELETE FROM sessions WHERE expires_at <= ?');
const stmtSessionsGetAllActive = db.prepare('SELECT sid, sess, expires_at FROM sessions WHERE expires_at > ? ORDER BY expires_at DESC');
const stmtSessionsDestroyUser = db.prepare("DELETE FROM sessions WHERE json_extract(sess, '$.user') = ?");
const stmtSessionsDestroyOtherUser = db.prepare("DELETE FROM sessions WHERE json_extract(sess, '$.user') = ? AND sid != ?");
const stmtSessionGetBySid = db.prepare('SELECT sid, sess FROM sessions WHERE sid = ?');
const stmtSessionDestroySingle = db.prepare('DELETE FROM sessions WHERE sid = ?');
const stmtSessionsDestroyAllExceptCurrent = db.prepare('DELETE FROM sessions WHERE sid != ?');
const stmtSessionsDestroyAll = db.prepare('DELETE FROM sessions');
const stmtSessionsCountUser = db.prepare("SELECT COUNT(*) AS count FROM sessions WHERE json_extract(sess, '$.user') = ? AND expires_at > ?");

const stmtUserSettingsGet = db.prepare('SELECT ui_mode, theme FROM user_settings WHERE username = ?');
const stmtUserSettingSetUiMode = db.prepare(`
...
`);
const stmtUserSettingSetTheme = db.prepare(`
...
const stmtVideoBySourceUrlGet = db.prepare('SELECT id, title, filename, thumbnail, duration, size, source_url, uploaded_by, uploaded_at FROM videos WHERE source_url = ? LIMIT 1');

const stmtPushSubSave = db.prepare(`
...
`);
const stmtPushSubsGet = db.prepare('SELECT * FROM push_subscriptions WHERE username = ?');
const stmtPushSubDelete = db.prepare('DELETE FROM push_subscriptions WHERE endpoint = ?');
const stmtPushSubsDeleteUser = db.prepare('DELETE FROM push_subscriptions WHERE username = ?');
const stmtPushSubsCleanupStale = db.prepare('DELETE FROM push_subscriptions WHERE last_used_at < ?');
const stmtPushSubTouch = db.prepare('UPDATE push_subscriptions SET last_used_at = ? WHERE endpoint = ?');

const stmtActivityLogsPrune = db.prepare(`
...
`);
const stmtOldActivityLogsClear = db.prepare(`
    DELETE FROM activity_logs
...
function clearOldActivityLogs(username) {
    if (!username) return;
    try {
        stmtOldActivityLogsClear.run(username, username);
    } catch (err) {
...
function setUserSetting(username, key, value) {
    ...
    try {
        ...
        if (key === 'ui_mode') {
            ...
            stmtUserSettingSetUiMode.run(username, cleanVal, now);
            return true;
        }
        if (key === 'theme') {
            const cleanVal = String(value);
            stmtUserSettingSetTheme.run(username, cleanVal, now);
            return true;
        }
        return false;
    } catch (err) {
...
function getVideoBySourceUrl(sourceUrl) {
    ...
    try {
        return stmtVideoBySourceUrlGet.get(sourceUrl) || null;
    } catch {
        return null;
    }
}
...
function savePushSubscription(username, subscription, userAgent = null) {
    ...
    try {
        ...
        const nowIso = new Date().toISOString();
        stmtPushSubSave.run(username, subscription.endpoint, keys.p256dh || '', keys.auth || '', userAgent, nowIso, nowIso);
        return true;
    } catch (err) {
...
function getPushSubscriptions(username) {
    if (!username) return [];
    try {
        return stmtPushSubsGet.all(username);
    } catch (err) {
...
function deletePushSubscription(endpoint) {
    ...
    try {
        stmtPushSubDelete.run(endpoint);
        return true;
    } catch (err) {
...
function deletePushSubscriptionsForUser(username) {
    if (!username) return 0;
    try {
        const result = stmtPushSubsDeleteUser.run(username);
        return result.changes || 0;
    } catch (err) {
...
function cleanupStalePushSubscriptions(maxAgeDays = 30) {
    try {
        ...
        const result = stmtPushSubsCleanupStale.run(cutoff);
        if (result.changes > 0) {
...
function touchPushSubscription(endpoint) {
    if (!endpoint) return;
    try {
        stmtPushSubTouch.run(new Date().toISOString(), endpoint);
    } catch {}
}

function pruneActivityLogs(maxKeep = 1500) {
    try {
        const result = stmtActivityLogsPrune.run(maxKeep);
        if (result.changes > 0) {
