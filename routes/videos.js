...
// POST /rename/:id — Rename video title (uploader or admin)
...
// POST /delete/:id — Delete video (uploader or admin)
    ...
    try {
        ...
        if (video) {
            if (req.session.user !== 'muaj' && video.uploaded_by !== req.session.user) {
                ...
                return res.status(403).render('forbidden', { user: req.session.user, message: 'Not permitted.' });
            }
