...
// POST /rename/:id — Rename video title (Owner or Admin only)
router.post('/rename/:id', isAuthenticated, (req, res) => {
    ...
    // Access control: Ensure user is admin ('muaj') or original uploader
    ...
    if (req.session.user !== 'muaj' && video.uploaded_by !== req.session.user) {
        return res.status(403).json({ error: 'Forbidden — Not permitted to rename this video.' });
    }
...
// POST /delete/:id — Delete video (Owner or Admin only)
router.post('/delete/:id', isAuthenticated, requireCsrf, async (req, res) => {
    ...
    // Access control: Ensure user is admin ('muaj') or original uploader
    const existingVideo = db.prepare('SELECT uploaded_by FROM videos WHERE id = ? OR filename = ?').get(videoId, videoId);
    if (existingVideo && req.session.user !== 'muaj' && existingVideo.uploaded_by !== req.session.user) {
        const isAjax = req.xhr ||
            (req.headers.accept && req.headers.accept.includes('application/json')) ||
            (req.headers['content-type'] && req.headers['content-type'].includes('application/json'));
        if (isAjax) {
            return res.status(403).json({ error: 'Forbidden — Not permitted to delete this video.' });
        }
        return res.status(403).send('Forbidden — Not permitted to delete this video.');
    }
